...
func TestGetAccountAPI(t *testing.T) {
	...
	testcases := []struct {
		...
		},
		{
			...
			accountID: account.ID,
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuthorization(t, request, tokenMaker, authorizationTypeBearer, "unathorized", user.Role, time.Minute)
			},
			buildstubs: func(mockStore *mockdb.MockStore) {
				mockStore.EXPECT().
					GetAccounts(gomock.Any(), gomock.Eq(account.ID)).
	...
	testCases := []struct {
	...
	}{
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				...
				store.EXPECT().
					ListAccounts(gomock.Any(), gomock.Eq(arg)).Times(1).Return(accounts, nil)
			},
			checkResponses: func(recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusOK, recorder.Code)
				requireBodyAccounts(t, recorder.Body, accounts)
			},
		},
		{
			name: "NoAuthorization",
